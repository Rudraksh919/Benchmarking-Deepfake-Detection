...
This module provides utilities for video processing, face detection, and extraction
from video files. It includes classes for video handling and face finding operations.

Key Components:
- Video: Basic video file interface
- FaceFinder: Advanced face detection and tracking in videos
- DataGenerator: Data loading and augmentation utilities
...
class Video:
    """
    Video file interface for frame-by-frame processing.
    
    Provides methods to navigate and extract frames from video files using imageio.
    ...
    Attributes:
        ...
        container: ImageIO video reader object
...
class FaceFinder(Video):
    """
    Advanced face detection and tracking in video files.
    
    Extends Video class to provide face detection, tracking, and extraction
    capabilities. Uses face_recognition library for robust detection and
    implements tracking to optimize performance across frames.
    
    Args:
        path (str): Path to the video file
        load_first_face (bool): Whether to detect face in first frame. Default: True
        
    Attributes:
        faces (dict): Dictionary storing extracted face images by frame number
        coordinates (dict): Face location and rotation information
        last_frame: Most recently processed frame
        frame_shape (tuple): Video frame dimensions (height, width)
        last_location (tuple): Last known face location for tracking
    """
...
class FaceBatchGenerator:
    ...
    def resize_patch(self, patch):
        m, n = patch.shape[:2]
        resized= zoom(patch, (self.target_size / m, self.target_size / n, 1))
...
def compute_accuracy(classifier, root_dir, frame_subsample_count=30):
    predictions = {}
    ...
    for subdir, _, files in os.walk(root_dir):
        for f in files:
            if f.lower().endswith(('.mp4', '.avi', '.mov')):
                video_paths.append(os.path.join(subdir, f))
    ...
    for idx, full_path in enumerate(video_paths, 1):
        video_name = os.path.basename(full_path)[:-4]

        try:
            face_finder = FaceFinder(full_path, load_first_face=True)
            skipstep = max(floor(len(face_finder) / frame_subsample_count), 1)
            face_finder.find_faces(resize=0.5, skipstep=skipstep)

            gen = FaceBatchGenerator(face_finder)
            preds = predict_faces(gen, classifier)

            if len(preds) > 0:
                predictions[video_name] = (np.mean(preds > 0.5), preds)

        except:
            pass  # skip broken or empty videos silently

        print(f"Progress: {idx}/{total} videos processed ({(idx/total)*100:.1f}%)")
